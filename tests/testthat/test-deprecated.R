...
test_that("opencage_forward/opencage_reverse return what they should.", {
  ...
  expect_deprecated(results <- opencage_forward(placename = "Sarzeau"))
  ...
  expect_deprecated(results <- opencage_forward(placename = "Islington, London"))
  ...
  expect_deprecated(results <- opencage_forward(placename = placename))
...
test_that(
  "opencage_forward/opencage_reverse return what they should
...
test_that("the bounds argument is well taken into account", {
  ...
  expect_warning(results1 <- opencage_forward(placename = "Berlin",
                                              key = Sys.getenv("OPENCAGE_KEY")))
  expect_warning(results2 <- opencage_forward(
    placename = "Berlin",
    bounds = c(-90, 38, 0, 45),
    key = Sys.getenv("OPENCAGE_KEY")
  ))
...
test_that("`opencage_key(quiet = FALSE)` messages", {
  ...
  expect_message(object = expect_warning(opencage_key(quiet = FALSE)),
