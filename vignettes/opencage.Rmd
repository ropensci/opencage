...
This package is an interface to the OpenCage API that allows forward and reverse geocoding. To use the package, you will need an API key. Register at https://opencagedata.com/pricing to get an API key. The
"Free Trial" plan provides up to 2,500 API requests a day. The geocoding
functions of the package will conveniently retrieve your API key with `oc_key()` if it is saved in the environment variable
`"OPENCAGE_KEY"`. For ease of use, save your API key as an environment variable in `.Renviron` as described at
...
Below are two simple examples. When building queries, OpenCage [best practices docs](https://opencagedata.com/api#bestpractices) can be very useful.
...
```{r, message=FALSE}
output2 <- oc_reverse(latitude = 51.5034070, 
                      longitude = -0.1275920)
output2
```
