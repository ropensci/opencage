...
The OpenCage API therefore supports about a dozen parameters to customise a query and here we will explain how to use them with {opencage} to get better geocoding results.

## Multiple results

Forward geocoding typically returns multiple results, because many places have the same or similar names.

`oc_forward_df()` only returns one result (the one defined as best result by OpenCage API) by default.
...
for_df
#>             location ccode
#> 1 Golden Gate Bridge    at
#> 2  Buckingham Palace    cg
#> 3       Eiffel Tower    be
