...
date: "2020-10-07"
description: >
  "The OpenCage API therefore supports about a dozen parameters to customise a query and here we will explain how to use them."
output: 
...
The OpenCage API therefore supports about a dozen parameters to customise a query and here we will explain how to use them with {opencage}.

## Multiple results

Forward geocoding typically returns multiple results, because many places have the same or similar names. 

`oc_forward_df()` only returns one result by default. 
...
#> 3 Berlin      39.8  -89.9 Berlin, Illinois, United States of America     
#> 4 Berlin      41.6  -72.7 Berlin, Connecticut, United States of America  
#> 5 Berlin      38.3  -75.2 Berlin, Maryland, United States of America
```


Reverse geocoding always returns [at most one result](https://opencagedata.com/api#ranking), therefore `oc_reverse_df()` does not support the `limit` argument.

OpenCage may sometimes have more than one record of one place. 
These are deduplicated by default. 
If you set the `no_dedupe` argument to `TRUE` you will receive duplicated results, if available. 
...
Place names are often ambiguous. 
...
They are only relevant and available for forward geocoding. 
...
E.g. "AR" for Argentina, "FR" for France, "NZ" for the New Zealand. 
...
#> 4 Paris       39.6  -87.7 Paris, Illinois, United States of America 
...
#> 3 Paris       43.5  12.1  Paris, Monterchi, Arezzo, Italy                   
#> 4 Paris       37.4  -8.79 8670-320 São Teotónio, Portugal                   
...
The bounds parameter can most easily be specified with the `oc_bbox()` helper, for example like `bounds = oc_bbox(-0.56, 51.28, 0.27, 51.68)`. 
OpenCage provides a '[bounds-finder](https://opencagedata.com/bounds-finder)' to interactively determine bounds values.

Below is an example of the use of `bounds` where the bbox given specifies the the South American contintent. 
...
#> 2 Paris     -13.5   -62.5 Canton Motegua, Municipio Baures, Provincia de Iténez, Bolivia
#> 3 Paris      10.6   -74.3 Paris, El Retén, MAG, Colombia                                
#> 4 Paris      -3.99  -79.2 110108, Loja, Ecuador                                         
#> 5 Paris     -38.4   -72.1 París, Victoria, Provincia de Malleco, Chile
```

Again, you can also use {opencage} to determine a bounding box for subsequent queries. 
If you wanted to map Hawaiian Airports for example, you could get their point coordinates like this:
...
#>    placename oc_lat oc_lng oc_formatted                                                                                     
#>    <chr>      <dbl>  <dbl> <chr>                                                                                            
#>  1 Airport     21.3  -158. Daniel K. Inouye International Airport, Noonan Street, Honolulu, HI 96819, United States of Amer~
#>  2 Airport     19.7  -156. Ellison Onizuka Kona International Airport at Keahole, Keahole Airport Road, Kailua-Kona, HI, Un~
#>  3 Airport     20.9  -156. Kahului Airport, Hana Highway, Lower Paia, HI 96779, United States of America                    
#>  4 Airport     19.7  -155. Hilo International Airport, Andrews Avenue, Hilo, HI 96720, United States of America             
#>  5 Airport     21.4  -158. Kaneohe Bay Marine Corp Air Station/Mokapu Point Airport, C Street, Kailua, HI 96863, United Sta~
#>  6 Airport     19.6  -156. Old Kona Airport State Recreation Area, Laniakea, Kailua-Kona, Hawaii, United States of America  
#>  7 Airport     21.2  -157. Molokai Airport, Airport Loop, Kualapu‘u, HI 96729, United States of America                     
#>  8 Airport     20.8  -157. Lanai Airport, Lanai Airport Road, Lanai City, HI 96763, United States of America                
#>  9 Airport     21.3  -158. Kalaeloa Airport, Saratoga Avenue, Kapolei, HI 96862, United States of America                   
#> 10 Airport     21.0  -157. Kapalua-West Maui Airport, Honoapiilani Highway, Napili, HI 96761, United States of America
```

Note that such a query will only give you aerodromes with "Airport" in their name or address, but not necessarily airfields, airstrips, etc. If you are more interested in these kind of features, you might want to take a look at the {[osmdata](https://docs.ropensci.org/osmdata/)} package.
...
A point is a named numeric vector of a latitude, longitude coordinate pair in decimal format. 
The `proximity` parameter can most easily be specified with the `oc_points()` helper, for example like `proximity = oc_point(38.0, -84.5)` if you happen to already know the coordinates. If not you can also look them up with {opencage}, of course:
...
#> 3 Paris       39.6 -87.7  Paris, Illinois, United States of America
...
I.e. in the following example, the French capital is too large to be returned. 
...
Besides parameters to target your search better, OpenCage offers parameters to receive more and specific information from the API.
...
Alternatively you can specify the "native" tag, in which case OpenCage will attempt to return the response in the "official" language(s).
...
In case the `language` parameter is set to `NULL` (which is the default), the tag is not recognized, or OpenCage does not have a record in that language, the results will be returned in English. 
...
Note however, that there are some language tags in use on OpenStreetMap for example, one of OpenCage's main sources, that do not conform with the IETF BCP 47 standard. 
...
It might therefore be helpful to consult the details page of the target country on openstreetmap.org in order to see which language tags are actually used. 
In any case, neither the OpenCage API nor the functions in this package will validate the language tags you provide.

For further details see [OpenCage's API documentation](https://opencagedata.com/api#language) on that subject. 

### Annotations

OpenCage supplies additional information about the result location in the so-called [annotations](https://opencagedata.com/api#annotations). 
...
Whether the annoations are shown, is controlled by the `no_annotations` argument. 
...
#> # A tibble: 1 x 71
#>   placename oc_lat oc_lng oc_confidence oc_formatted oc_mgrs oc_maidenhead oc_callingcode oc_flag oc_geohash oc_qibla
#>   <chr>      <dbl>  <dbl>         <int> <chr>        <chr>   <chr>                  <int> <chr>   <chr>         <dbl>
#> 1 Dublin      53.3  -6.26             5 Dublin, Ire~ 29UPV8~ IO63ui83sw               353 "\U000~ gc7x9812v~     114.
#> # ... with 60 more variables: oc_wikidata <chr>, oc_dms_lat <chr>, oc_dms_lng <chr>, oc_itm_easting <chr>,
#> #   oc_itm_northing <chr>, oc_mercator_x <dbl>, oc_mercator_y <dbl>, oc_osm_edit_url <chr>, oc_osm_note_url <chr>,
...
#> #   oc_currency_subunit <chr>, oc_currency_subunit_to_unit <int>, oc_currency_symbol <chr>, oc_currency_symbol_first <int>,
#> #   oc_currency_thousands_separator <chr>, oc_roadinfo_drive_on <chr>, oc_roadinfo_speed_in <chr>,
#> #   oc_sun_rise_apparent <int>, oc_sun_rise_astronomical <int>, oc_sun_rise_civil <int>, oc_sun_rise_nautical <int>,
#> #   oc_sun_set_apparent <int>, oc_sun_set_astronomical <int>, oc_sun_set_civil <int>, oc_sun_set_nautical <int>,
#> #   oc_timezone_name <chr>, oc_timezone_now_in_dst <int>, oc_timezone_offset_sec <int>, oc_timezone_offset_string <chr>,
#> #   oc_timezone_short_name <chr>, oc_what3words_words <chr>, oc_northeast_lat <dbl>, oc_northeast_lng <dbl>,
#> #   oc_southwest_lat <dbl>, oc_southwest_lng <dbl>, oc_iso_3166_1_alpha_2 <chr>, oc_iso_3166_1_alpha_3 <chr>,
#> #   oc_category <chr>, oc_type <chr>, oc_city <chr>, oc_continent <chr>, oc_country <chr>, oc_country_code <chr>,
#> #   oc_county <chr>, oc_county_code <chr>, oc_political_union <chr>, oc_state <chr>, oc_state_code <chr>
```
...
It is `FALSE` by default, which means OpenCage will not attempt to match the nearest road. Some road and driving information is nevertheless provided as part of the annotations (see above), if `roadinfo` is set to `FALSE`. 


```r
oc_forward_df(placename = c("Europa Advance Rd", "Bovoni Rd"), roadinfo = TRUE)
#> # A tibble: 2 x 29
#>   placename oc_lat oc_lng oc_confidence oc_formatted oc_roadinfo_dri~ oc_roadinfo_one~ oc_roadinfo_road oc_roadinfo_roa~
#>   <chr>      <dbl>  <dbl>         <int> <chr>        <chr>            <chr>            <chr>            <chr>           
#> 1 Europa A~   36.1  -5.34             9 Europa Adva~ right            no               Europa Advance ~ secondary       
#> 2 Bovoni Rd   18.3 -64.9              9 Bovoni Hill~ left             <NA>             <NA>             <NA>            
#> # ... with 20 more variables: oc_roadinfo_speed_in <chr>, oc_roadinfo_surface <chr>, oc_northeast_lat <dbl>,
#> #   oc_northeast_lng <dbl>, oc_southwest_lat <dbl>, oc_southwest_lng <dbl>, oc_iso_3166_1_alpha_2 <chr>,
#> #   oc_iso_3166_1_alpha_3 <chr>, oc_category <chr>, oc_type <chr>, oc_continent <chr>, oc_country <chr>,
#> #   oc_country_code <chr>, oc_postcode <chr>, oc_road <chr>, oc_road_type <chr>, oc_state <chr>, oc_county <chr>,
#> #   oc_natural <chr>, oc_state_code <chr>
```
...
The geocoding functions also have an `abbr` parameter, `FALSE` by default. 
...
#>   placename   oc_lat oc_lng oc_formatted                                                                
#>   <chr>        <dbl>  <dbl> <chr>                                                                       
#> 1 Wall Street   40.7  -74.0 Wall Street, Manhattan Community Board 1, NY 10005, United States of America
oc_forward_df("Wall Street", abbrv = TRUE)
#> # A tibble: 1 x 4
#>   placename   oc_lat oc_lng oc_formatted                                       
#>   <chr>        <dbl>  <dbl> <chr>                                              
#> 1 Wall Street   40.7  -74.0 Wall St, Manhattan Community Board 1, NY 10005, USA
```
...
All function arguments mentioned above are vectorised, so you can send queries like this:
...
Or geocode placenames in a data frame:
...
#>   <chr>             <chr>  <dbl>  <dbl> <chr>                                                                               
#> 1 Golden Gate Brid~ at     48.0   15.6  Karer, Golden Gate Bridge, 3180 Gemeinde Lilienfeld, Austria                        
#> 2 Buckingham Palace cg     -4.80  11.8  Buckingham Palace, Boulevard du Général Charles de Gaulle, Pointe-Noire, Republic o~
#> 3 Eiffel Tower      be     50.9    4.34 Eiffel Tower, Avenue de Bouchout - Boechoutlaan, Brussels, Belgium
```

That also works with `oc_reverse_df()`, of course. 


```r
rev_df <- data.frame(
  lat = c(51.952659, 41.401372, 0),
  lon = c(7.632473, 2.128685, 0))
...
For further information about the output and the query parameters, see the [API docs](https://opencagedata.com/api) and the [OpenCage FAQ](https://opencagedata.com/faq). When building queries, OpenCage's [best practices](https://opencagedata.com/api#bestpractices) can be very useful, as well as their guide to [geocoding accuracy](https://opencagedata.com/guides/how-to-think-about-geocoding-accuracy).

- we will show examples with `_df` functions - refer to outputs vignette for other output options. All function arguments mentioned here are also available in the `oc_forward()` and `oc_reverse()` (if applicable) functions.
- vectorised arguments of the geocoding functions
